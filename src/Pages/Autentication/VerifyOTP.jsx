...
const VerifyOTP = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      setDisable(true);
      response = await verifyOtp({ email: location?.state?.email, otp: otp });
      if (response?.success) {
        displaySuccessToast(response?.message || "OTP Verified Successfully");
        navigate(routeConst.resetPass, {
          state: { email: location?.state?.email },
        });
      } else {
        displayErrorToast(
          response?.message || "Something went wrong While verifying OTP"
        );
      }
    } catch (error) {
  ...
  const handleResend = async () => {
    try {
      ...
      setOtp("");
      setKey(key + 1);
      const response = await sendOtp({ email: location?.state?.email });
      if (response?.success) {
        displaySuccessToast(response?.message || "OTP reent Successfully");
      } else {
        displayErrorToast(
